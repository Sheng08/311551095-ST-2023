...
class CSS:
    ...
    def __check_course_format(self, course):
        ...
        if not course[1] in self.__workdays:  # Day
            raise TypeError
        if not (type(course[2]) is int and type(course[3]) is int
                and 1 <= course[2] and course[2] <= 8 and 1 <= course[3]
                and course[3] <= 8 and course[2] <= course[3]):  # Time
            raise TypeError
...
# public attributes
    ...
    def add_course(self, course):
        ...
        if self.check_course_exist(course) == False:
            return False
        for cus in self.__courses:
            if not (cus[1] != course[1] or cus[3] < course[2]
                    or cus[2] > course[3]):
                return False
